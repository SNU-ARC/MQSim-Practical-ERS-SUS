...
namespace SSD_Components
{
		...
		eraseReasonableSuspensionTimeForWrite(EraseReasonableSuspensionTimeForWrite), opened_scheduling_reqs(0)
	{
		...
		std::cout << EraseSuspensionEnabled <<" : " << ProgramSuspensionEnabled << std::endl;
	...
	{
#if (1)
		flash_chip_ID_type nChip_ID = _my_instance->Round_robin_turn_of_channel[channelID];
		...
		{
			NVM::FlashMemory::Flash_Chip* chip = _my_instance->_NVMController->Get_chip(channelID, nChip_ID);
			
			//A transaction has been started, so TSU should stop searching for another chip
	#if (ADAPTIVE_RPS_SCH_ENABLE)	
			if (true == _my_instance->service_read_transaction(chip, READ_PRIO_SCH))
	#else
			if (true == _my_instance->service_read_transaction(chip, true))
	#endif
			{
				assert(_my_instance->_NVMController->Get_channel_status(channelID) != BusChannelStatus::IDLE);
				_my_instance->Round_robin_turn_of_channel[channelID] = (flash_chip_ID_type)(nChip_ID_ORG + 1) % _my_instance->chip_no_per_channel;
				break;
			}
			nChip_ID = (flash_chip_ID_type)(nChip_ID + 1) % _my_instance->chip_no_per_channel;
		}

		for ( unsigned int i = 0; i < _my_instance->chip_no_per_channel; i++)
		{
			NVM::FlashMemory::Flash_Chip* chip = _my_instance->_NVMController->Get_chip(channelID, i);
			if ( true == _my_instance->check_user_read_transaction(chip) )
			{
	...
	#endif
		{
			if (_my_instance->_NVMController->Get_channel_status(channelID) == BusChannelStatus::IDLE)
			{
				...
				for (unsigned int i = 0; i < _my_instance->chip_no_per_channel; i++) 
				{
					...
					//A transaction has been started, so TSU should stop searching for another chip
					if (true == _my_instance->service_read_transaction(chip, false))
					{
						assert(_my_instance->_NVMController->Get_channel_status(channelID) != BusChannelStatus::IDLE);
						_my_instance->Round_robin_turn_of_channel[channelID] = (flash_chip_ID_type)(nChip_ID_ORG + 1) % _my_instance->chip_no_per_channel;
						break;
		...
		for (unsigned int i = 0; i < _my_instance->chip_no_per_channel; i++) {
			...
			if (!_my_instance->service_read_transaction(chip, false))
				if (!_my_instance->service_write_transaction(chip))
					_my_instance->service_erase_transaction(chip);
			_my_instance->Round_robin_turn_of_channel[channelID] = (flash_chip_ID_type)(_my_instance->Round_robin_turn_of_channel[channelID] + 1) % _my_instance->chip_no_per_channel;
	...
	#if (ADAPTIVE_RPS_SCH_ENABLE)				
			if ( false == _my_instance->service_read_transaction(chip, READ_PRIO_SCH))
	#else
			if ( false == _my_instance->service_read_transaction(chip, true))
	#endif
			{
				bool checkNonUserRead = true;
				flash_channel_ID_type nCh_ID = chip->ChannelID;
				flash_chip_ID_type nChip_ID_ORG = chip->ChipID;
				flash_chip_ID_type nChip_ID = (flash_chip_ID_type)(nChip_ID_ORG + 1) % _my_instance->chip_no_per_channel;
				for (unsigned int i = 0; i <  _my_instance->chip_no_per_channel; i++)
				{				
					NVM::FlashMemory::Flash_Chip* chip = _my_instance->_NVMController->Get_chip(nCh_ID, i);
					if ( true == _my_instance->check_user_read_transaction(chip) )
					{
						checkNonUserRead = false;
						break;
					}
				}

	#if (ADAPTIVE_RPS_SCH_ENABLE)						
				if ( (false == READ_PRIO_SCH) || (true == checkNonUserRead) )
	#else
				if ( true == checkNonUserRead )
	#endif
				{
					NVM::FlashMemory::Flash_Chip* chip = _my_instance->_NVMController->Get_chip(nCh_ID, nChip_ID_ORG);
					if (!_my_instance->service_read_transaction(chip, false))
						if (!_my_instance->service_write_transaction(chip))
							_my_instance->service_erase_transaction(chip);
				}
